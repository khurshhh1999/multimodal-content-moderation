...
async def accept_image(
    *,
    ...
    """Hash, store, persist, and enqueue one image. Caller already rate-limited."""
    ...
    job_id = uuid.uuid4()
    object_key = f"content/{digest[:2]}/{digest}/{content_id}.{object_extension(content_type)}"
    bucket = (
        settings.gcs_bucket
            ...
            try:
                with start_span("ingest.persist_job"):
                    ...
                    await conn.execute(
                        ...
                        """,
                        job_id,
                        f'{{"content_hash":"{digest}","tenant_id":"{tenant_id}","source":"{source}"}}',
                    )
            except Exception as exc:  # noqa: BLE001
...
def apply_rate_limit_headers(response: Response, headers: dict[str, str]) -> None:
